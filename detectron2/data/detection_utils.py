...
def annotations_to_instances(annos, image_size, mask_format="polygon"):
    ...
    # Kaihong's edit: handle cases when category_id is a list for multi-label annotation
    if len(annos) and isinstance(annos[0]["category_id"], list):
        classes_lists = [obj["category_id"] for obj in annos]
        classes_item = list(itertools.chain(*classes_lists))
        classes_ind = [[ind] * len(classes_lists[ind]) for ind in range(len(classes_lists))]
        classes_ind = list(itertools.chain(*classes_ind))
        classes_item = torch.tensor(classes_item, dtype=torch.int64)
        classes_ind = torch.tensor(classes_ind, dtype=torch.int64)
        classes = torch.stack([classes_item, classes_ind], dim=1)
    else:
