...
class FastRCNNOutputLayers(nn.Module):
    ...
    def from_config(cls, cfg, input_shape):
        return {
            ...
            "multi_label"               : cfg.MODEL.ROI_BOX_HEAD.MULTI_LABEL,
            # fmt: on
        }
    ...
    def losses(self, predictions, proposals):
        ...
        if self.use_sigmoid_ce:
            if self.multi_label:
                gt_classes_multi_label = []
                gt_cls_indices = []
                indices_start = 0
                for proposal in proposals:
                    this_gt_classes = proposal.gt_classes
                    if proposal.has("ori_gt_classes"):
                        ori_gt_classes = proposal.ori_gt_classes
                        ori_gt_boxes = proposal.ori_gt_boxes.tensor
                        this_gt_boxes = proposal.gt_boxes.tensor if proposal.has("gt_boxes") else proposal.proposal_boxes.tensor
                        this_gt_boxes = this_gt_boxes * (this_gt_classes != self.num_classes).unsqueeze(1).float()
                        equal = (this_gt_boxes.unsqueeze(1) == ori_gt_boxes.unsqueeze(0)).all(-1)
                        equal_indices = equal.nonzero(as_tuple=False)
                        gt_cls_indices.append(equal_indices[:, 0] + indices_start)
                        gt_classes_multi_label.append(ori_gt_classes[equal_indices[:, 1]])
                    indices_start += len(this_gt_classes)
                gt_cls_indices = torch.cat(gt_cls_indices) if len(gt_cls_indices) > 0 else torch.empty(0).long()
                gt_classes_multi_label = torch.cat(gt_classes_multi_label) if len(gt_classes_multi_label) > 0 else torch.empty(0).long()
                loss_cls = self.sigmoid_cross_entropy_loss(scores, gt_classes_multi_label, gt_cls_indices)
            else:
    ...
    # with slight modifications
    # Kaihong's edit: allow to pass gt_cls_indices to calculate loss for multi-label case
    def sigmoid_cross_entropy_loss(self, pred_class_logits, gt_classes, gt_cls_indices=None):
        """
        Args:
        ...
        if gt_cls_indices is not None:
            target[gt_cls_indices, gt_classes] = 1
        else:
        ...
        if self.use_fed_loss:
            assert gt_cls_indices is None, "Not implemented for gt_cls_indices"
