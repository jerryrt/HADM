...
""",
    ...
    parser.add_argument("--tta", action="store_true", help="use tta for eval")
...
def default_writers(
        output_dir: str, max_iter: Optional[int] = None,
        use_wandb=False,
    ):
    """
    ...
    Args:
        ...
        use_wandb: whether to use wandb writer for logging
    ...
    if use_wandb:
        writers.append(WAndBWriter(output_dir=output_dir))
    return writers
...
## Kaihong's edit: DefaultInferencer takes model as init input and image as input to __call__
class DefaultInferencer:
    """
    Create a simple end-to-end predictor with the given model that runs on
    single device for a single input image.
